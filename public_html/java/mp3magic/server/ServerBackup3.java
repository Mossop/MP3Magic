...
// deletetrack
		...
		case 9: 
// findtrack
			System.out.println( " find track - NOT IMPLEMENTED YET!" ); 
/*
			try {
				// ######################################### CUM BACK
				String a[] = { p[1], p[3], p[5], p[7], p[9], p[11], p[13], p[15] }				
				temp = msg.findTrack( p[1], p[3], null, null, null, null, null, null )

				msgError = 110 // if no matches
			} catch (SQLException e) {
				msgError = 111;
			}			
*/
...
// addtrack
			...
			try {
				temp = msg.findTrack( p[1], p[2], null, null, null, -21, -21, -21 );
				if ( temp.hasNext() ) { 
